...
import Firebase
    ...
    struct FirebaseStruct {
        ...
        init(user: String, timestamp: Int, xCoordinate: Double, yCoordinate: Double, location: String) {
            ...
            //let ref = Database.database().reference().child("MAIN").childByAutoId()
            let ref = Database.database().reference().child("MAIN").child(translateUnixTime(time: getCurrentUnixTime())).child((Auth.auth().currentUser?.uid)!).childByAutoId()
            ref.setValue(XYData)
